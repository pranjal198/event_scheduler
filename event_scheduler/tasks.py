...
def debug_task_custom_1(self):
    ...
    try:
        events=Task.objects.filter(remainder_date=datetime.now().date())
        for event in events:
...
def debug_task_daily(self):
    ...
    email_from=settings.EMAIL_HOST_USER
    try:
        events=Task.objects.filter(remainder="Daily",date__gte=datetime.now().date())
        for event in events:
            profile_objs=Profile.objects.filter(batch=event.target_batch,department=event.target_branch)
            subject='Remainder for the event- '+event.title
            print(subject)
        ...
        events_2=Task.objects.filter(remainder="Week before",date=datetime.now().date()+timedelta(days=7))
...
def debug_task_weekly(self):
    ...
    email_from=settings.EMAIL_HOST_USER
    try:
        events=Task.objects.filter(remainder="Weekly",date__gte=datetime.now().date())
        for event in events:
            profile_objs=Profile.objects.filter(batch=event.target_batch,department=event.target_branch)
            subject='Remainder for the event- '+event.title
            print(subject)
...
def debug_task_monthly(self):
    ...
    email_from=settings.EMAIL_HOST_USER
    try:
        events=Task.objects.filter(remainder="Monthly",date__gte=datetime.now().date())
        for event in events:
            profile_objs=Profile.objects.filter(batch=event.target_batch,department=event.target_branch)
            subject='Remainder for the event- '+event.title
            print(subject)
